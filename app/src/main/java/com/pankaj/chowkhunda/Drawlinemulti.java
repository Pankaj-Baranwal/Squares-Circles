 ...
 */
public class Drawlinemulti extends View {
    ...
    public Drawlinemulti(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void setupDrawing() {
        ...
        f = new float[2];
        g = new float[2];
        elastmove = new float[undo * 2];
        slastmove = new float[undo * 2];
        aheadlme = new float[60];
        aheadlms = new float[60];
        rand = new Random();
        radius = 10;
        drawPath = new Path();
        drawPaint = new Paint();
    ...
    public float[] closepoint(float lx, float ly) {
        for (int ia = cols - 2; ia >= 1; ia--) {
            if (lx >= (xcoords[ia] - xdiff) && lx <= (xcoords[ia] + xdiff)) {
                ex = xcoords[ia];
            } else if (lx <= (xcoords[1] - xdiff)) ex = xcoords[0];
            else if (lx >= (xcoords[cols - 1] - xdiff)) ex = xcoords[cols - 1];
        }
        for (int j = rows - 2; j >= 1; j--) {
            if (ly <= (ycoords[j] + ydiff) && ly >= (ycoords[j] - ydiff)) {
                ey = ycoords[j];
            } else if (ly <= (ycoords[1] - ydiff)) ey = ycoords[0];
            else if (ly >= (ycoords[rows - 1] - ydiff)) ey = ycoords[rows - 1];
        }
        return new float[]{ex, ey};
    }

    public void line(float gx, float gy, float hx, float hy) {
        if (gx == hx || gy == hy) {
    ...
    public void undo(float gx, float gy, float hx, float hy) {
        if(larst==0) {
            slastmove[0] = gx;
            slastmove[1] = gy;
            elastmove[0] = hx;
            ...
            aheadlms[(larst-1)*2]=gx;
            aheadlms[((larst-1)*2)+1]=gy;
            aheadlme[(larst-1)*2]=hx;
            aheadlme[((larst-1)*2)+1]=hy;
            larst+=1;
        }
    }
